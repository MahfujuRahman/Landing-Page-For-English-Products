...
function set_upazila(district_id = 47) {
    ...
    let filtered = upazilas.filter(upazila => upazila.district_id == district_id);

    let district = districts.find(d => d.id == district_id);
    document.querySelector('input[name="zila_name"]').value = district.bn_name ?? '';
...
function set_upazila_name(upazila_id) {
    ...
    document.querySelector('input[name="upazila_name"]').value = upazila.bn_name;
    renderCartTable();
}
...
function setItem(action, buttonElement) {
    ...
    const cart_item_total_el = parentDiv.parentElement.querySelector('.cart-item-total');
...
function add_to_cart(productId) {
    ...
    const checkbox = document.getElementById(`p${productId}`);
    const qtyInput = checkbox.closest('.product_item_parent').querySelector('input[type="number"]');
...
function renderCartTable() {
        ...
        tableHTML += `
            <tr>
                <td>
                    ...
                    <input type="hidden" name="cart[${index}][qty]" value="${item.quantity}"/>
                    <i class="bi bi-trash text-danger me-2" onclick="removeItem(${item.id})"></i>
                    ${item.name}
                </td>
                <td>${item.quantity}</td>
    ...
    tableHTML += `
            ...
            <tfoot>
                ...
                <tr>
                    <td colspan="2" class="text-end">
                        ${global_discount > 0 ? `(${global_discount}%) Discount` : 'Discount'}
                    </td>
                ...
                <tr>
                    <td colspan="2" class="text-end shipping_area_td">
                        ...
                        <div class="text-nowrap">
                            Shipping charge (${shipping_area} dhaka ${shipping_charge}৳ KG)
                        </div>
                    </td>
                    <td>+ ${total_shipping_charge.toFixed(2)} ৳</td>
                </tr>
                <tr>
                    <td colspan="2" class="text-end">
                        <b> Grand total </b>
